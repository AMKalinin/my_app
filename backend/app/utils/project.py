...
class ProjectWorker():
    ...
    def add_tasks(self, hdf:h5py.File, images:list[UploadFile]):
        for index, image in enumerate(images):
            self.add_task(hdf, image, index)

    def create_project(self, images:list[UploadFile]) -> None:
        with h5py.File(self.project_path, 'w-') as hdf:
            self.add_tasks(hdf, images)
